...
@interface WorksFangViewController ()<UICollectionViewDataSource,UICollectionViewDelegate>
{
...
@implementation WorksFangViewController
...
-(void)loadData
{
    ...
    NSMutableDictionary* Submit_data = [NSMutableDictionary dictionary];
    NSString* s_page = [NSString stringWithFormat:@"%ld",(long)page];
    [Submit_data setObject:s_page forKey:@"page"];
    UILog(@"%@",url);
    ASIHTTPRequest* request = [HttpRequest requestPost:url Dic:Submit_data authorization:YES];
        ...
        if (infos.count!=0) {
            ...
            for (int i=0; i<count; i++){
                ...
                if (!strIsEmpty([d objectForKey:@"s_image"]))
                {
                    ...
                    NSString* s = [NSString stringWithFormat:@"%@",[d objectForKey:@"s_image"]];
                    model.s_image = s;
                };
                ...
                {
                    //model.s_image =[NSString stringWithFormat:@"%@",[d objectForKey:@"image"]];
                    NSString* s = [NSString stringWithFormat:@"%@",[d objectForKey:@"mgid"]];
                    model.s_mgid = s;
                };
                if (!strIsEmpty([d objectForKey:@"gdprice"]))
                {
                    //model.s_image =[NSString stringWithFormat:@"%@",[d objectForKey:@"image"]];
                    NSString* s = [NSString stringWithFormat:@"%@",[d objectForKey:@"gdprice"]];
                    model.s_gdprice = s;
                };
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (imgArr.count!=0)
    {
        ...
        //UILog(@"model.s_image %@",model.s_image);
        [cell.imgView setImageWithURL:url placeholderImage:[Photo imageWithColorString:model.fillColor]];
        //cell.text.text = [NSString stringWithFormat:@"价钱: %@",model.s_gdprice];
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    return CGSizeMake((_Screen_Width-6)/3,(_Screen_Width-6)/3);
}
