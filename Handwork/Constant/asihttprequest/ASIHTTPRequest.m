...
// 在内存中缓存的凭据,用于当useSessionPersistence是肯定的
...
//我们跟踪cookie我们已经收到所以我们可以从sharedHTTPCookieStorage后删除它们
...
//持久连接,我们看这里,试图找到一个现有的连接到相同的服务器目前不使用
...
/*
 是的,当带宽节流是活跃的
 　　这个标志并不表示是否开启节流——而它是否正在使用中
 　　它不将被设置为当节流与setShouldThrottleBandwidthForWWAN打开,但wi - fi连接处于活动状态
 */
...
/*
   当是的,带宽使用WWAN时将自动进行节流(3 g /优势/ GPRS)
 　 Wifi将不会被压制
 */
...
/*
 / /这个锁确保代表只收到一个通知需要身份验证
 　　/ /使用ASIAuthenticationDialogs时,这也保证了一次只显示一个对话框
 　　/ /如果请求不能立即获得锁,这意味着一个对话框被显示或委托处理身份验证的挑战
 　　/ /一旦被锁,它将试图寻找现有的凭证,而不是显示对话框/通知委托方
 　　/ /这是因此可以利用任何凭证提供给另一个请求,如果他们是合适的
 */
...
//当调节带宽,将在未来的日期,我们将允许所有请求醒来,重新安排他们的流
...
// 您可以使用[ASIHTTPRequest setShouldUpdateNetworkActivityIndicator没有):如果你想管理它自己
...
/*
 另外,覆盖showNetworkActivityIndicator / hideNetworkActivityIndicator
 　　默认这并没有在Mac OS X上,但同样覆盖上面的方法对不同的行为
 */
...
- (void)buildPostBody
{
	...
	if ([self postBodyFilePath]) {
        ...
        //如果我们写信给《华盛顿邮报》的身体通过appendPostData或appendPostDataFromFile,关闭写流
...
// 调用这个方法作为NSString接收的数据。不要使用二进制数据!
...
- (void)main
{
	@try {
        ...
        // ASINetworkQueue的生成HEAD请求可能已经设置错误。如果是这样的话,我们不应该继续。
		if ([self error]) {
			[self setComplete:YES];
			[self markAsFinished];
			return;		
        ...
        // 如果我们重定向,我们已经有一个CFHTTPMessageRef
		if (request) {
			CFRelease(request);
		}
        ...
        //如果这是一个由ASINetworkQueue生成HEAD请求,我们需要让主要先请求生成它的头,所以我们可以使用它们
		if ([self mainRequest]) {
        ...
        // 即使这是一个与mainRequest HEAD请求,我们仍然需要调用给子类一个机会来添加自己的领导请求(ASIS3Request这)
		...
		if ([self downloadCache]) {
            ...
            // 如果缓存数据陈旧,或我们被告知要问服务器是否已被修改,我们需要添加标题为条件
        ...
        // 否则,我们将开始下载代理PAC文件,并调用startRequest一旦完成这一过程
...
- (void)applyAuthorizationHeader
{
	...
	if (![[self requestHeaders] objectForKey:@"Authorization"]) {
        ...
        // 如果我们有基本身份验证显式地设置和一组用户名和密码请求,添加一个基本认证头
		if ([self username] && [self password] && [[self authenticationScheme] isEqualToString:(NSString *)kCFHTTPAuthenticationSchemeBasic]) {
			...
			if ([self useSessionPersistence]) {
				...
				if (credentials) {
                    ...
                    // 当认证密钥集,凭证存储身份验证后的挑战,所以我们可以让CFNetwork应用它们
					...
					if ([credentials objectForKey:@"Authentication"]) {
                        ...
                        // 如果我们已经跟这个服务器和有效身份证件,我们将它们应用于请求
                    ...
                    // 如果没有设置认证密钥,这些凭证存储一组用户名和密码后,前一个请求通过基本身份验证
...
- (void)buildRequestHeaders
{
    ...
    // 建立和设置用户代理字符串如果请求已经没有指定一个自定义的用户代理
...
// 设置WWAN连接下的request的流量上限为50KB limit
