...
+ (MBProgressHUD *)showHUDAddedToEx:(UIView *)view animated:(BOOL)animated withText:(NSString*)text detailText:(NSString*)detaiText
{	
    MBProgressHUD *mHUD = [self showHUDAddedTo:view animated:animated withText:text detailText:detaiText];    
    CGRect frame = CGRectMake(0, view.frame.size.height/3, view.frame.size.width, view.frame.size.height/3);
    mHUD.frame = frame;
    //NSLog(@"MBProgressHUD-> height %0.0f",view.frame.size.height/3);
    return mHUD;
}


+(MBProgressHUD*)showHUDAddedTo:(UIView *)view animated:(BOOL)animated withText:(NSString*)text detailText:(NSString*)detaiText{
	...
	hud.labelText = text;
	hud.detailsLabelText = detaiText;
	[view addSubview:hud];
	[hud show:animated];
...
- (id)initWithView:(UIView *)view {
    ...
    //CGRect rect = CGRectMake(0, view.frame.size.height/3, view.frame.size.width, view.frame.size.height/3);
	id me = [self initWithFrame:rect];
...
- (void)show:(BOOL)animated {
    ...
    // 超时自动隐藏
    [self performSelector:@selector(hideDelayed:) withObject:nil afterDelay:40];
}
