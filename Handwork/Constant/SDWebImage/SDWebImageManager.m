...
- (void)imageCache:(SDImageCache *)imageCache didFindImage:(UIImage *)image forKey:(NSString *)key userInfo:(NSDictionary *)info
{
    ...
    if ([delegate respondsToSelector:@selector(webImageManager:didFinishWithImage:forURL:forUserInfo:)])
    {
        ...
        //  不能直接使用objc_msgSend的原型方法来匿名调用，需要显性做强制转换再调用，否则在64bit下会出现异常【参考：http://my.oschina.net/vimfung/blog/341119 】
        //objc_msgSend(delegate, @selector(webImageManager:didFinishWithImage:forURL:), self, image, url);
        
        void (*action)(id, SEL, SDWebImageManager *, UIImage *, NSURL *, NSDictionary *) = (void (*)(id, SEL, SDWebImageManager *, UIImage *, NSURL *, NSDictionary *)) objc_msgSend;
        action(delegate, @selector(webImageManager:didFinishWithImage:forURL:forUserInfo:), self, image, url, userInfo);
